...
who_std <- tibble(
  ...
  std_pop = c(17917, 17802, 17701, 17613, 17536, 17470,
              17414, 17366, 17327, 17293, 17263, 17233,
              17199, 17160, 17115, 17069, 17018, 16952,
              16866, 16765, 16656, 16546, 16434, 16323,
              16212, 16094, 15971, 15851, 15735, 15621,
              15504, 15376, 15233, 15070, 14890, 14702,
              14510, 14307, 14091, 13865, 13629, 13391,
              13163, 12949, 12745, 12538, 12320, 12091,
              11845, 11585, 11320, 11048, 10757, 10443,
              10113, 9774, 9435, 9095, 8757, 8423, 8087,
              7750, 7425, 7113, 6812, 6517, 6221, 5923,
              5618, 5311, 5005, 4706, 4412, 4125, 3844,
              3569, 3300, 3035, 2773, 2518, 2271, 2033,
              1807, 1594, 1392, 1203, 1025, 863, 718,
              589, 1950 # This last value for age 90. If it's for 85+, adjust age range. Assuming it's for age 90.
...
confounders <- c("age", "sex", "bmi", "on_med_htn", "current_smoke", "a1c_imp", "sod_pot_imp")
...
for (def_name in names(bp_definitions)) {
  ...
  for (bp_measure_col_name in names(bp_measurement_types_map)) {
    ...
    if(nrow(data_for_rates) == 0) {
        message(paste("    Skipping rates for", def_name, "-", bp_measure_label, "due to no data after NA removal for BP category."))
        next
    }
    ...
    actual_bp_levels_in_data <- sort(unique(data_for_rates[[bp_measure_col_name]]))
    ...
    tidy_cvd_hr  <- tibble(bp_level = character(), cvd_hr = numeric(), cvd_ci_low = numeric(), cvd_ci_high = numeric(), cvd_hr_ci = character())

    if (nrow(data_for_hr) > 0 && n_distinct(data_for_hr[[bp_measure_col_name]]) > 0) {
      data_for_hr[[bp_measure_col_name]] <- factor(data_for_hr[[bp_measure_col_name]])
      ...
      if (!ref_level %in% levels(data_for_hr[[bp_measure_col_name]])) {
        if (length(levels(data_for_hr[[bp_measure_col_name]])) > 0) {
          ...
          message(paste("    Warning for HRs ('", def_name, "', '", bp_measure_label,
                        "'): 'Normal' category not found. Using '", ref_level, "' as reference.", sep=""))
        } else {
          ref_level <- NA
        }
      }
      ...
      if (!is.na(ref_level)) {
        ...
        if (nrow(model_data_mort) > 0 && n_distinct(model_data_mort[[bp_measure_col_name]]) > 1) {
          tryCatch({
            model_mortality <- coxph(
              formula = as.formula(paste("Surv(time_to_event, mortality_event) ~",
                                         bp_measure_col_name, "+",
            ...
            tidy_mort_hr <- bind_rows(
              tibble(bp_level = ref_level, mort_hr = 1, mort_ci_low = NA,
                     mort_ci_high = NA, mort_hr_ci = "1.00 (Reference)",
                     mort_p_val = NA),
              tidy_mort_hr
            )
          }, error = function(e) {
            message(paste("    Error fitting mortality Cox model for", def_name, "-", bp_measure_label, ":", e$message))
            tidy_mort_hr <<- tibble(bp_level = levels(data_for_hr[[bp_measure_col_name]]), 
                                    mort_hr = NA_real_, mort_ci_low = NA_real_, mort_ci_high = NA_real_, 
                                    mort_hr_ci = NA_character_, mort_p_val = NA_character_)
          })
        } else {
          tidy_mort_hr <- tibble(bp_level = levels(data_for_hr[[bp_measure_col_name]]), 
                                 mort_hr = NA_real_, mort_ci_low = NA_real_, mort_ci_high = NA_real_, 
        ...
        if (nrow(model_data_cvd) > 0 && n_distinct(model_data_cvd[[bp_measure_col_name]]) > 1) {
          tryCatch({
            model_cardio <- coxph(
              formula = as.formula(paste("Surv(time_to_all_cardio, all_cardio_event) ~",
                                         bp_measure_col_name, "+",
                                         paste(confounders, collapse = " + "))),
              data = model_data_cvd
            )
            ...
            tidy_cvd_hr <- bind_rows(
              tibble(bp_level = ref_level, cvd_hr = 1, cvd_ci_low = NA,
                     cvd_ci_high = NA, cvd_hr_ci = "1.00 (Reference)",
                     cvd_p_val = NA),
              tidy_cvd_hr
            )
          }, error = function(e) {
            message(paste("    Error fitting CVD Cox model for", def_name, "-", bp_measure_label, ":", e$message))
            tidy_cvd_hr <<- tibble(bp_level = levels(data_for_hr[[bp_measure_col_name]]), 
                                   cvd_hr = NA_real_, cvd_ci_low = NA_real_, cvd_ci_high = NA_real_, 
                                   cvd_hr_ci = NA_character_, cvd_p_val = NA_character_)
          })
        } else {
          tidy_cvd_hr <- tibble(bp_level = levels(data_for_hr[[bp_measure_col_name]]), 
                                cvd_hr = NA_real_, cvd_ci_low = NA_real_, cvd_ci_high = NA_real_, 
        ...
        tidy_mort_hr <- tibble(bp_level = actual_bp_levels_in_data, 
                               mort_hr = NA_real_, mort_ci_low = NA_real_, mort_ci_high = NA_real_, 
                               mort_hr_ci = NA_character_, mort_p_val = NA_character_)
        tidy_cvd_hr  <- tibble(bp_level = actual_bp_levels_in_data, 
                               cvd_hr = NA_real_, cvd_ci_low = NA_real_, cvd_ci_high = NA_real_, 
                               cvd_hr_ci = NA_character_, cvd_p_val = NA_character_)
      }
    } else {
    ...
    all_results_list[[paste(def_name, bp_measure_col_name, sep = "_")]] <- combined_stats_for_measure
  }
}
...
create_forest_plot <- function(data, definition, outcome = "mort") { #Change outcome to "cvd" for CVD plots
  ...
  plot_data <- data %>%
    ...
    mutate(
      ...
      crude_rate = if_else(is.na(.data[[paste0(outcome, "_crude_rate")]]), "-", sprintf("%.2f", .data[[paste0(outcome, "_crude_rate")]])),
      std_rate = if_else(is.na(.data[[paste0(outcome, "_rate")]]), "-", sprintf("%.2f", .data[[paste0(outcome, "_rate")]])),
      hr_text = if_else(is.na(.data[[paste0(outcome, "_hr")]]), "-", sprintf("%.2f", .data[[paste0(outcome, "_hr")]])),
      ci_text = if_else(is.na(.data[[paste0(outcome, "_ci_low")]]), "Reference", sprintf("(%.2f-%.2f)", .data[[paste0(outcome, "_ci_low")]], .data[[paste0(outcome, "_ci_high")]])),
      lower_plot = if_else(is.na(.data[[paste0(outcome, "_ci_low")]]), .data[[paste0(outcome, "_hr")]], .data[[paste0(outcome, "_ci_low")]]),
  ...
  # Add header
  header <- c("BP Category", "n", "N", "Crude Rate", "Standard Rate", "HR (95% CI)", "p value")
...
# Save the plot
ggsave(file.path(output_dir, "cardiovascular_rates_forest_plots.png"), combined_plot, width = 15, height = 10)