...
# Including kilifi and JHS definitions
# Define BP categorization functions for each definition
bp_definitions <- list(
  "ESC" = function(data) {
    data %>%
  ...
  },                            # Change here to add more definitions
  "JHS" = function(data) {
    data %>%
      mutate(
        clinic_cat = case_when(
          spot_bp_sys < 140 & spot_bp_dys < 90 ~ "Normal",
          spot_bp_sys >= 140 | spot_bp_dys >= 90 ~ "Hypertension"
        ),
        amb_cat = case_when(
          idaco_sbpbr_avg < 135 & idaco_dia_avg < 80 |
            idaco_day_sbpbr < 140 & idaco_day_dia < 85 |
            idaco_night_sbpbr < 130 & idaco_night_dia < 75 ~ "Normal",
          idaco_sbpbr_avg >= 135 | idaco_dia_avg >= 80 |
            idaco_day_sbpbr >= 140 | idaco_day_dia >= 85 |
            idaco_night_sbpbr >= 130 | idaco_night_dia >= 75 ~ "Hypertension"
        )
      )
  },                                       
  "Kilifi" = function(data) {
    data %>%
      mutate(
        clinic_cat = case_when(
          spot_bp_sys < 134 & spot_bp_dys < 87 ~ "Normal",
          spot_bp_sys >= 134 | spot_bp_dys >= 87 ~ "Hypertension"
        ),
        amb_cat = case_when(
          idaco_sbpbr_avg < 124 & idaco_dia_avg < 70 |
            idaco_day_sbpbr < 139 & idaco_day_dia < 84 |
            idaco_night_sbpbr < 116 & idaco_night_dia < 64 ~ "Normal",
          idaco_sbpbr_avg >= 124 | idaco_dia_avg >= 70 |
            idaco_day_sbpbr >= 139 | idaco_day_dia >= 84 |
            idaco_night_sbpbr >= 116 | idaco_night_dia >= 64 ~ "Hypertension"
        )
      )
  }
)
...
custom_colours <- c("Normal" = "#00FF00",      # Green
                    "Elevated BP" = "#FFA500",    # Amber
                    "Hypertension" = "#FF0000"#, # Red
                    #"amb_cat=Normal" = "#00FF00",          # Green
                    #"amb_cat=Elevated BP" = "#FFA500",     # Amber
                    #"amb_cat=Hypertension" = "#FF0000"     # Red
                    )
...
# Save the combined plot as a PNG
ggsave(
  filename = "combined_nelson_aalen.png",
  plot = combined_plot,
  path = "..\\Manuscript\\final_manuscript_results\\Figures",  # Replace with your desired file path
  width = 20,                     # Width in inches (adjust as needed)
  height = 30                     # Height in inches (adjust as needed)
)
