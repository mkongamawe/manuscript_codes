...
df_clinic <- data.frame(x = dens_clinic$x, y = dens_clinic$y,
                        group = "Clinic SBP")
...
df_night <- data.frame(x = dens_night$x, y = dens_night$y,
                       group = "Nighttime SBP")
...
# Create the plot
p <- ggplot() +
  geom_area(data = df_clinic, aes(x = x, y = y), fill = "#fec44f", alpha = 0.2) +
  geom_area(data = df_night, aes(x = x, y = y), fill = "#a6cee3", alpha = 0.3) +

  geom_area_pattern(data = shade_clinic, aes(x = x, y = y),
                    fill = "#fec44f", alpha = 0.5,
                    pattern = "stripe", pattern_angle = 45,
                    pattern_density = 0.05, pattern_spacing = 0.02,
                    pattern_color = "#d95f02", pattern_fill = NA) +
  geom_area_pattern(data = shade_night, aes(x = x, y = y),
                    fill = "#a6cee3", alpha = 0.6,
                    pattern = "stripe", pattern_angle = -45, # Negative angle flips the stripes #nolint
                    pattern_density = 0.05, pattern_spacing = 0.02,
                    pattern_color = "#1f78b4", pattern_fill = NA) +
  # Add the main density outline curves on top
  geom_line(data = df_all, aes(x = x, y = y, color = group, linetype = group), linewidth = 1.2) +

  geom_segment(aes(x = t_clinic, xend = t_clinic, y = 0, yend = y_max,
                   color = "Clinic threshold >= 140 mmHg", 
                   linetype = "Clinic threshold >= 140 mmHg"), linewidth = 1) +
  geom_segment(aes(x = t_night, xend = t_night, y = 0, yend = y_max,
                   color = "Nighttime threshold >= 120 mmHg", 
                   linetype = "Nighttime threshold >= 120 mmHg"), linewidth = 1) +
  
  annotate("text", x = 100, y = 0.009, label = label_night,
           color = "#1f78b4", fontface = "italic", size = 4.5, hjust = 0.5) +
  annotate("curve", x = 105, y = 0.010, xend = 118, yend = 0.0115, # Adjust X/Y coordinates as needed based on your actual data
           curvature = -0.2, arrow = arrow(length = unit(0.2, "cm")), color = "#1f78b4") +

  # Clinic text and curved arrow
  annotate("text", x = 150, y = 0.0075, label = label_clinic, 
           color = "#d95f02", fontface = "italic", size = 4.5, hjust = 0.5) +
  annotate("curve", x = 155, y = 0.0085, xend = 142, yend = 0.0105, # Adjust X/Y coordinates as needed based on your actual data
           curvature = 0.2, arrow = arrow(length = unit(0.2, "cm")), color = "#d95f02") +

  scale_x_continuous(breaks = seq(80, 250, by = 20)) +
  scale_y_continuous(breaks = seq(0, 0.015, by = 0.002)) +

  scale_color_manual(name = "",
                     values = c("Clinic SBP" = "#d95f02",
  ...
  theme_classic() +
  labs(title = "Deaths (n=118)",
       x = "Systolic Blood Pressure (mmHg)",
       y = "Density",
       color = "") +
  theme(legend.position = c(0.8, 0.9), # Moves legend inside the plot
