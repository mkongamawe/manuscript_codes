...
for (bp_col in names(bp_columns)) {
  ...
  sd_value <- round(sd(updated_combined_data[[bp_col]], na.rm = TRUE), 1)
 ...
 updated_combined_data_1 <-updated_combined_data |>
  ...
  surv_object <- Surv(time =updated_combined_data_1$time_to_all_cardio, event =updated_combined_data_1$all_cardio_event)
...
fp_set_style(box = c("royalblue", "#CC5500"),
             line = c("darkblue", "#FF7043"),
...
all_all_mortality_dia <- wrap_plots(p1, p2, nrow = 2) +
  plot_annotation(title = "All-cause mortality (n = 118)",
                    theme = theme(plot.title = element_text(size = 30, hjust = 0.5)))
...
# Save the plot
ggsave(file.path(output_dir, "New all cardiovascular per SD.png"),
...
sys_mortality_mod_1_plot <- out_data |>
...
sys_mortality_mod_2_plot <- out_data |>
  forestplot(mean = estimate,
    lower = conf.low,
    upper = conf.high,
    labeltext = c(`BP Measurement`, "HR (95% CI)", "p value"),
...
dia_mortality_mod_1_plot <- out_data |>
  forestplot(mean = estimate,
    lower = conf.low,
    upper = conf.high,
    labeltext = c(`BP Measurement`, "HR (95% CI)", "p value"),
...
dia_mortality_mod_2_plot <- out_data |>
  forestplot(mean = estimate,
    lower = conf.low,
    upper = conf.high,
    labeltext = c(`BP Measurement`, "HR (95% CI)", "p value"),
...
#All all cardiovascular event plots
p1 <- grid2grob(print(sys_mortality_mod_1_plot))
p2 <- grid2grob(print(sys_mortality_mod_2_plot))
p3 <- grid2grob(print(dia_mortality_mod_1_plot))
p4 <- grid2grob(print(dia_mortality_mod_2_plot))


mod_1_mortality_dia <- wrap_plots(p1, p3, nrow = 2) +
    plot_annotation(title = "Model 1: All-cause mortality (n = 118)",
                    theme = theme(plot.title = element_text(size = 30, hjust = 0.5)))

mod_2_mortality_dia <- wrap_plots(p2, p4, nrow = 2) +
    plot_annotation(title = "Model 2: All-cause mortality (n = 118)",
                    theme = theme(plot.title = element_text(size = 30, hjust = 0.5)))
#all_all_mortality_dia

################################
#
all_all_plots <- ggarrange(mod_1_mortality_dia, mod_2_mortality_dia, ncol = 2)
...
# Save the plot
ggsave(file.path(output_dir, "New all mortality only per SD.png"),
