...
all_cardio_systolic_cox <- tbl_merge(
  tbls = list(cox_systolic_combined, all_cardio_mortality),
  tab_spanner = FALSE
) %>%
  as_gt() %>%
  gt::tab_spanner(
  ...
  gt::gtsave(
    filename = "all_systolic_study_interaction.DOCX",
    path = "C:\\Users\\cmwagwabi\\OneDrive - Kemri Wellcome Trust\\Documents - Anthony Etyang's files\\ABPM and HDSS events Data_Shared with Clement\\Project Folder - Clement\\Manuscript\\results_v7"
  )
...
#Combining all cardiovascular systolic BP
all_cardio_diastolic_cox <- tbl_merge(
  tbls = list(cox_diastolic_combined, all_cardio_mortality_dia),
  tab_spanner = FALSE
) %>%
  as_gt() %>%
  gt::tab_spanner(
  ...
  gt::gtsave(
    filename = "all_diastolic_study_interaction.DOCX",
    path = "C:\\Users\\cmwagwabi\\OneDrive - Kemri Wellcome Trust\\Documents - Anthony Etyang's files\\ABPM and HDSS events Data_Shared with Clement\\Project Folder - Clement\\Manuscript\\results_v7"
  )
