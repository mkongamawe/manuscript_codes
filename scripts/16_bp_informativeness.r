...
informativeness <- sapply(results, function(x) {
  round((x / spot_bp_sys_chi2) * 100)
})
...
################################
systolic_informativenes <- full_join(
  informative1_sys,
  informative2_sys,
  by = "measure"
)
...
systolic_informativenes |>
  ...
  cols_label(
    measure = md(""),
    confounder_adjusted_chi2_statistic.x = md(
      "Confounder-adjusted _&chi;<sup>2</sup>_ statistic "
    ),
    informativeness.x = md("Informativeness "),
    confounder_adjusted_chi2_statistic.y = md(
      "Confounder-adjusted _&chi;<sup>2</sup>_ statistic"
    ),
  ...
  tab_footnote(
    ...
    locations = cells_column_labels(c(
      confounder_adjusted_chi2_statistic.x,
      confounder_adjusted_chi2_statistic.y
    ))
  ) |>
  tab_footnote(
    footnote = md(
      "Informativeness of the given measure(as indicated by the confounder-adjusted &chi;<sup>2</sup> statistic)
  ...
  tab_caption(
    caption = md(
      "Relative informativeness of different systolic blood pressure indices for all-cause mortality and all cardiovascular events"
    )
  ) |>
  gt::gtsave(
    filename = "informativeness_systolic.docx",
    path = "results/tables"
  )

############################################################################
diastolic_informativeness <- full_join(
  informative1_dys,
  informative2_dys,
  by = "measure"
)
...
diastolic_informativeness |>
  ...
  tab_caption(
    caption = md(
      "Relative informativeness of different diastolic blood pressure indices for all-cause mortality and all cardiovascular events"
    )
  ) |>
  gt::gtsave(
    filename = "informativeness_diastolic.docx",
    path = "results/tables"
  )
