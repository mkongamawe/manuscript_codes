...
updated_combined_data$Weight <- factor(updated_combined_data$Weight, 
...
compute_overall_hrs <- function(data, event_var, time_var) {
  ...
  for (bp_col in names(bp_columns)) {
    ...
    model <- coxph(as.formula(paste("Surv(", time_var, ", ", event_var, ") ~ bp_scaled + age + sex + bmi + on_med_htn + current_smoke + a1c_imp + sod_pot_imp")), 
                   data = data_temp)
...
compute_subgroup_hrs <- function(data, subgroup_var, subgroup_levels, formula_base, 
                                 event_var, time_var) {
  ...
  for (sg in subgroup_levels) {
    ...
    for (bp_col in names(bp_columns)) {
      ...
      model_formula <- as.formula(paste("Surv(", time_var, ", ", event_var, ") ~ bp_scaled +", formula_base))
...
prepare_forest_data <- function(combined_df) {
  ...
  bp_order <- c("Clinic SBP", "Daytime SBP", "Nighttime SBP", "24h SBP",
                "Clinic DBP", "Daytime DBP", "Nighttime DBP", "24h DBP")
  
  wide_data <- combined_df |>
    select(subgroup, bp_label, estimate, conf.low, conf.high, p_value_str, N, n) |>
    pivot_wider(
      id_cols = c(bp_label),
      names_from = subgroup,
  ...
  out_data <- wide_data |>
    ...
    mutate(
      ...
      name = sapply(strsplit(name, "_"), function(x) paste(x[1:(length(x)-1)], collapse = "_"))
    ) |>
    ...
    mutate(
      ...
      `BP Measurement` = if_else(row_number() == 1, as.character(bp_label),
                                     paste0("   ", group)),
    ...
    shape_map <- setNames(available_shapes[seq_along(unique_groups)], unique_groups)
...
run_analysis <- function(event_var, time_var, outcome_name) {
  ...
  age_plot <- create_forest_plot(out_data_age, paste(outcome_name, "- Age Subgroups"))
  
  # 2. Forest plot for Weight subgroups
  overall_weight <- compute_overall_hrs(updated_combined_data, event_var, time_var)
  ...
  weight_plot <- create_forest_plot(out_data_weight, paste(outcome_name, "- Weight Subgroups"))
  ...
  sex_plot <- create_forest_plot(out_data_sex, paste(outcome_name, "- Sex Subgroups"))
...
mortality_results <- run_analysis("mortality_event", "time_to_event", "All Mortality")

# Run cardiovascular analysis if requested
if(include_cardio) {
  cardio_results <- run_analysis("all_cardio_event", "time_to_all_cardio", "Cardiovascular Events")
}
