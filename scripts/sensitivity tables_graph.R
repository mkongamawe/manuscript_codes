...
# Results without KILIFI and JHS
     ...
     get_sens_spec <- function(data, sys_var, dia_var, sys_cut, dia_cut) {
         ...
         sens_fmt <- sprintf("%d(%d, %d)", round(100 * sens), round(100 * sens_ci[1]), round(100 * sens_ci[2]))
         
         spec <- TN / (TN + FP)
         spec_ci <- binom.test(TN, TN + FP)$conf.int
         spec_fmt <- sprintf("%d(%d, %d)", round(100 * spec), round(100 * spec_ci[1]), round(100 * spec_ci[2]))
     ...
     for (def in names(bp_thresholds)) {
         ...
         for (type in names(bp_vars)) {
             ...
             result <- get_sens_spec(updated_combined_data, vars[1], vars[2], cuts[1], cuts[2])
     ...
     make_section_table <- function(df, title) {
         ...
         # Reorder columns
         df <- df[, c("Definition", "spA", "24hr", "sp0", "Nighttime", "sp1", "Daytime", "sp2", "Clinic", "sp3")]
         ...
         ft <- flextable(df) %>%
             set_header_labels(
                 ...
                 Daytime = "Daytime",
                 Clinic = "Clinic",
                 spA = "", sp0 = "", sp1 = "", sp2 = "", sp3 = "" # spacer columns have no labels
             ) %>%
             ...
             border(j = c("spA", "sp0", "sp1", "sp2", "sp3"), border = fp_border(color = "white"), part = "all") %>%
            
              # Add bottom border to spacer columns  on the second row of the body
             hline(i=1, j = c("spA", "sp0", "sp1", "sp2", "sp3"), border = fp_border(width = 1), part = "header") %>%
             
             # Add bottom border to spacer columns  on the last row of the body
             hline(i = nrow(df), j = c("spA", "sp0", "sp1", "sp2", "sp3"), border = fp_border(width = 1), part = "body") %>%
     ...
     print(doc, target = "..\\Manuscript\\final_manuscript_results\\HTN_Validity_Table_1_spacer.docx")
     ...
     sensitivity_long <- sensitivity_tbl %>%
         pivot_longer(cols = -Definition, names_to = "Measurement", values_to = "Sensitivity_CI") %>%
         mutate(
     ...
     custom_colors <- c("24hr"="#0072B2",
                          "Nighttime" = "#e31a1c",
                         "Daytime" = "#33a02c",
                         "Clinic" = "#ff7f00")
        
    p <- ggplot(sensitivity_long, aes(x = Measurement, y = Sensitivity, color = Measurement)) +
         geom_point(size = 3,shape=15) +
         geom_errorbar(aes(ymin = CI_Lower, ymax = CI_Upper), width = 0.1) +
         facet_wrap(~Definition, ncol = 2) +
         labs(
             title = "Prognostic Sensitivity by Measurement Method",
             x = "Measurement Method", y = "Sensitivity (%)"
         ) +
    ...
    # Reshape the specificit table to long format by definition
     specificity_long <- specificity_tbl %>%
         pivot_longer(cols = -Definition, names_to = "Measurement", values_to = "Specificity_CI") %>%
         mutate(
...
# Results with KILIFI and JHS
     ...
     ##### generate graph for sensitivities
     # Reshape the sensitivity table to long format by definition
     sensitivity_long <- sensitivity_tbl %>%
         pivot_longer(cols = -Definition, names_to = "Measurement", values_to = "Sensitivity_CI") %>%
         mutate(
     ...
     sensitivity_long$Definition <- factor(
         sensitivity_long$Definition,
         levels = c("Kilifi", "ESH/ISH/ESC", "ACC/AHA", "JHS")
     )
     ...
     ggplot(sensitivity_long, aes(x = Measurement, y = Sensitivity, color = Measurement)) +
         geom_point(size = 3,shape=15) +
         geom_errorbar(aes(ymin = CI_Lower, ymax = CI_Upper), width = 0.1) +
         facet_wrap(~Definition, nrow = 2) +
         labs(
             title = "Prognostic Sensitivity by Measurement Method",
             x = "Measurement Method", y = "Sensitivity (%)"
         ) +
     ...
     ggsave("prognostic_sensitivity_plot.png",
            height = 6, width = 10,
            path = "..\\Manuscript\\final_manuscript_results\\Figures")