...
for (bp_col in names(bp_columns)) {  
  ...
  # Calculate the schoenfeld residuals
  cox_diag[[bp_col]] <- cox.zph(cox_model)
...
ggsave("all mortality per half SD.png",
  ...
  ################################
# ----------------------Only all cardiovascular events plots---------------------
####################################
###########################################
#all_cardio model 1
# List of blood pressure columns and their labels
...
combined_data <- bind_rows(
  forest_data |>mutate(group = "Model 1"),
  forest_data_2 |>mutate(group = "Model 2")#,
  #forest_data_3 |>mutate(group = "Model 3")
) %>%
  select(label, group, estimate, conf.low, conf.high, p_value_str) %>%
  pivot_wider(
    id_cols = label,
...
# ----------------------Only all respiratory events plots---------------------
###########################################
#all_cardio model 1
# List of blood pressure columns and their labels
bp_columns <- list(
...
for (bp_col in names(bp_columns)) {
  ...
  # Fit a Cox proportional hazards model adjusted for age, BMI, HypertensionMeds, and CurrentSmoker
  surv_object <- Surv(time = updated_combined_data_1$time_to_all_resp, event = updated_combined_data_1$all_resp_event)
...
for (bp_col in names(bp_columns)) {
  ...
  # Fit a Cox proportional hazards model adjusted for age, BMI, HypertensionMeds, and CurrentSmoker
  surv_object <- Surv(time = updated_combined_data_1$time_to_all_resp, event = updated_combined_data_1$resp_event)
...
all_cardio_plot_2 <- out_data |>
  forestplot(mean = c(estimate),
             ...
             clip = c(0.5, 2.5),
             col = fpColors(box = "#1c5c8f", line = "#1c5c8f"),
             #title = "All Cardiovascular Events (n = 27)",
             xticks = c(0.5, 1, 1.5, 2.0, 2.5),
             graph.pos = 2,
             txt_gp = fpTxtGp(label = gpar(cex = 1.2),
                              xlab = gpar(cex = 1.2),
...
all_all_cardio_dia <- wrap_plots(p1, p2, nrow = 2) +
    plot_annotation(title = "All respiratory events (n = 27)",
                    theme = theme(plot.title = element_text(size = 30, hjust = 0.5)))

ggsave("all respiratory per half SD.png",
       path = "results/figures",
       height = 30, width = 30, units = "cm",
       dpi = 400)