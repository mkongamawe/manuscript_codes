...
for (bp_col in names(bp_columns)) {
  ...
  # Fit a Cox proportional hazards model adjusted for age, BMI, HypertensionMeds, and CurrentSmoker
  surv_object <- Surv(
    time = updated_combined_data_1$time_to_all_cardio,
    event = updated_combined_data_1$all_cardio_event
  )
...
all_all_cardio_dia <- wrap_plots(p1, p2, nrow = 2) +
  plot_annotation(
    title = "All cardiovascular events (n = 39)",
    theme = theme(plot.title = element_text(size = 30, hjust = 0.5))
  )

ggsave(
  "all cardiovascular per half SD.png",
  path = "results/figures",
  height = 30,
  width = 30,
  units = "cm",
...
for (bp_col in names(bp_columns)) {
  ...
  # Fit a Cox proportional hazards model adjusted for age, BMI, HypertensionMeds, and CurrentSmoker
  surv_object <- Surv(
    time = updated_combined_data_1$time_to_all_resp,
    event = updated_combined_data_1$all_resp_event
  )
...
for (bp_col in names(bp_columns)) {
  ...
  surv_object <- Surv(
    time = updated_combined_data_1$time_to_all_resp,
    event = updated_combined_data_1$resp_event
  )
...
all_all_cardio_dia <- wrap_plots(p1, p2, nrow = 2) +
  plot_annotation(
    title = "All respiratory events (n = 27)",
    theme = theme(plot.title = element_text(size = 30, hjust = 0.5))
  )

ggsave(
  "all respiratory per half SD.png",
  path = "results/figures",
  height = 30,
  width = 30,
  units = "cm",
