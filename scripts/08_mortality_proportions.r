...
create_guideline_plot <- function(guideline) {
  ...
  plot_data <- categorized_data %>%
    ...
    mutate(
      ...
      type = recode(type, clinic_cat = "Clinic", amb_cat = "Ambulatory")
    )
  ...
  p <- ggplot(
    plot_data,
    aes(x = type, y = mortality_rate, fill = bp_category)
  ) +
...
legend_only_plot <- ggplot(
  data.frame(
    type = factor(
      c("Normal", "Elevated BP", "Hypertension"),
      levels = c("Normal", "Elevated BP", "Hypertension")
    )
  ),
