...
project_table_1 <- updated_combined_data |>
    mutate(
        ...
        iso_clin_sys_hyp = if_else(
            spot_bp_sys >= 134 & spot_bp_dys < 87,
            "yes",
            "no"
        ),
        iso_amb_sys_hyp = if_else(
            idaco_sbpbr_avg >= 124 & idaco_dia_avg < 70,
            "yes",
            "no"
        ),
        iso_clin_dia_hyp = if_else(
            spot_bp_sys < 134 & spot_bp_dys >= 87,
            "yes",
            "no"
        ),
        iso_amb_dia_hyp = if_else(
            idaco_sbpbr_avg < 124 & idaco_dia_avg >= 70,
            "yes",
            "no"
        )
...
project_table_1 |>
    ...
    gt::gtsave(
        filename = "Table_1_vital_status_strat.docx",
        path = "results/tables"
    )
