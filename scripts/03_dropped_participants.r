...
filtered_out <- project_master_1 %>%
  select("pk_person", "updated_time", "study_no", "sex", "age", "agegrp", "height", "weight", #Delete updated time from this.
          "bmi", "muac", starts_with("idaco"), "spot_bp_sys", "spot_bp_dys", "spot_pulse", "prev_diag_htn",
          "on_med_htn", "current_smoke", "a1c", "self_dm", "dob", "agecat5", "agecat3", "study", "Urine_sodium_",
          "Urine_potassium")  %>%
  rename("sodium" = Urine_sodium_,
         "potassium" = Urine_potassium) %>%
  mutate(
...
filtered_out_2 <- filtered_out_1 %>%
  mutate(
    idaco_bp_phenotype = case_when(
      idaco_good == 1 & spot_bp_sys < 140 & spot_bp_dys < 90 &
        idaco_sbpbr_avg < 130 & idaco_dia_avg < 80 ~ "Normotensive",
      idaco_good == 1 & (spot_bp_sys >= 140 | spot_bp_dys >= 90) &
...
levels(filtered_out_2$current_smoke)[levels(filtered_out_2$current_smoke) == ""] <- "No"
...
shinda2_master_1[shinda2_master_1$pk_person %in% n_occur$var1[n_occur$Freq > 1], ]
...
filt_desc <- filtered_out_3 |>
  summarise(
    ...
    spot_incomplete = sum(idaco_good == 1 & (is.na(spot_bp_sys) | is.na(spot_bp_dys)), na.rm = TRUE)
  )
